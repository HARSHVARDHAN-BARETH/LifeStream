...
// Example context or hook for user info
// const useAuth = () => {
//     // Retrieve the laboratoryId from localStorage
//     const laboratoryId = localStorage.getItem('laboratoryId');

//     return { laboratoryId };
// };

const LaboratoryDonorRegister = () => {
    ...
    const laboratoryId = localStorage.getItem('laboratoryId');  // Assume hospitalId is stored in localStorage after login
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        axios.post('http://localhost:3000/laboratory-donor-register', { ...formData, laboratoryId })
            .then(response => {
    ...
    return (
      ...
      <h1 style={{ textShadow: '2px 2px 4px rgba(0, 0, 0, 0.5)' }} className="text-3xl font-bold text-white mb-6">Laboratory</h1>
        ...
        <Link to="/LaboratoryDashboard"><img src={homebutton} style={{ width: '40px', height: '40px' }} alt="Home" /></Link>
        </h1>
      </div>
        ...
        <form onSubmit={handleSubmit} className='space-y-4'>
                ...
                <button
                    type="submit"
                    style={buttonBackground}  className="text-white text-center text-2xl py-2 px-2 rounded-full w-56 "                >
                    Register Donor
                </button>
                </div>
            </form>
...
export default LaboratoryDonorRegister;
